...
ax.plot(br, exp1.conditions['find']['p_uncont'], 'o-', color=COLORS['find'],
        label='Find uncontaminated', linewidth=2, markersize=5)
ax.plot(br, exp1.conditions['avoid']['p_uncont'], 's--', color=COLORS['avoid'],
        label='Avoid contaminated', linewidth=2, markersize=5)

style_axis(ax, 'Base rate of contamination', 'P("Which are uncontaminated?")', (0.05, 0.95))
plt.tight_layout()
plt.show()
```
...
for ax, (dt, title) in zip(axes, [('singleton', 'Singleton selection'), ('set_id', 'Set identification')]):
    ax.plot(br, exp2.conditions[dt]['find_p'], 'o-', color=COLORS['find'],
            label='Find uncontaminated', linewidth=2, markersize=6)
    ax.plot(br, 1 - np.array(exp2.conditions[dt]['avoid_p']), 's--', color=COLORS['avoid'],
            label='Avoid contaminated', linewidth=2, markersize=6)
    style_axis(ax, 'Base rate of contamination', 'P("Which are uncontaminated?")', (0.15, 0.85), title)

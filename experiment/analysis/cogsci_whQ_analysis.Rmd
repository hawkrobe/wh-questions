...
dat_exp1 <-
  ...
  filter(!prolific_pid %in% c("66463ba8fac8591a6734db5b")) %>% #returned
  pivot_wider(
...
dat_exp1 <-
  ...
  select(prolific_pid, goal_condition, base_rate, raw_response, trial_id, decision_structure, `survey-text`) %>%
  mutate(base_rate = factor(base_rate),
         goal_condition = factor(goal_condition, 
                                 levels = c("uncont", "cont")),
...
dat_exp2 <-
  ...
  mutate(goal_condition = factor(goal_condition, levels = c("uncont", "cont")),
...
```{r saving tidied csv files}
write_csv(dat_exp1_coded, "exp1_coded.csv")
write_csv(dat_exp1_toCode, "exp1_toCode.csv")
write_csv(dat_exp2_sing, "exp2_singleton.csv")
write_csv(dat_exp2_setID, "exp2_setID.csv")
```
...
dat_exp1_gp <-
  ...
  summarize(p_which_uncont = mean(chose_which_uncontaminated, na.rm = TRUE),
            pwu_margin = qt(0.975, df = nrow(dat_exp1_coded) - 1) * sd(chose_which_uncontaminated) / sqrt(nrow(dat_exp1_coded))) 
...
dat_exp2_sing_gp <-
  ...
  summarize(p_which_uncont = mean(chose_which_uncontaminated, na.rm = TRUE),
            pwu_margin = qt(0.975, df = nrow(dat_exp2_sing) - 1) * sd(chose_which_uncontaminated) / sqrt(nrow(dat_exp2_sing))) 
...
dat_exp2_setID_gp <-
  ...
  group_by(goal_condition) %>%
  summarize(p_which_uncont = mean(chose_which_uncontaminated, na.rm = TRUE),
            pwu_margin = qt(0.975, df = nrow(dat_exp2_setID) - 1) * sd(chose_which_uncontaminated) / sqrt(nrow(dat_exp2_setID))) 
...
```{r regression}
mod_exp1 <- 
  glm(chose_which_uncontaminated ~ goal_condition*base_rate, data = dat_exp1_coded,
      family = "binomial",
      contrasts = list(goal_condition = "contr.sum", base_rate = "contr.sum"))
...
```{r}
mod_exp2 <- 
  glm(chose_which_uncontaminated ~ goal_condition*decision_structure, data = dat_exp2,
      family = "binomial",
